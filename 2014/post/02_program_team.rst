...
川田です。
今年も、Development Sprints というハッカソンに似た開発イベントを最終日に開催しました。
会場は、日本マイクロソフトさんの品川オフィスを提供していただきました。
連休最終日でしたが、50人以上の方に参加していただきました。

開発スプリントは、スプリントリーダーがテーマを出して参加者は興味が有るところで黙々と開発をするイベントです。
事前準備としては、スプリントリーダーと参加者の登録を受け付ける connpass を用意しました。
